...
class Calendar(Slide):
    ...
    def __init__(self, parent=None):
        ...
        self.icalGrabber = urllib3.PoolManager()
        
    def setup(self, ical_url):
        self.calendarUrl = ical_url
        self.timezone = timezone("Europe/Amsterdam")
        try:
            cal_req = self.icalGrabber.request('GET', self.calendarUrl) # grab calendar file from server
        except:
            return 1
        
        if cal_req.status == 200:
            print(cal_req.headers['content-type'])
            self.display_calendar = cal.Calendar().from_ical(cal_req.data)
            for event in self.display_calendar.walk("VEVENT"):
                if type(event.get("dtstart").dt) == datetime:
                    if event.get("dtstart").dt.date() <= date.today() and self.timezone.localize(datetime.utcnow()) < event.get("dtend").dt.astimezone(self.timezone):
                        #if event.get("dtstart").dt <= date.today() <= event.get("dtend").dt:
                        myAppointment = Appointment()
                        particiTitle = str(event.get("summary")).split(":")
                        if len(particiTitle) > 1:
                            myAppointment.setTitle(particiTitle[1])
                            myAppointment.setParticipants(particiTitle[0])
                        else:
                            myAppointment.setTitle(str(event.get("summary")))
